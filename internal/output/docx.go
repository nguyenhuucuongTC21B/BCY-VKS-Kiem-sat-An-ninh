...
netsh interface set interface "Wi-Fi" disable</code>`)
        ...
        sb.WriteString(`<table><tr><th>Loại</th><th>Model</th><th>VID/PID</th><th>Ổ</th><th>Số lần cắm</th><th>Lần đầu</th><th>Lần cuối</th><th>BadUSB</th></tr>`)
        for _, r := range ctx.Result.Bang4 {
                bad := "Không"
                if r.BadUSBWarning {
                        bad = "CÓ!"
                }
                fmt.Fprintf(&sb, `<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%d</td><td>%s</td><td>%s</td><td>%s</td></tr>`,
                        r.DeviceType, r.VendorModel, r.VIDPID, r.DriveLetter,
                        r.PlugCount, orDash(r.FirstPlug), orDash(r.LastPlug), bad)
        }
        sb.WriteString(`</table>`)

        // Lịch sử từng lần kết nối (text, tối đa 20 lần mỗi thiết bị)
        for _, r := range ctx.Result.Bang4 {
                if len(r.Sessions) == 0 {
                        continue
                }
                fmt.Fprintf(&sb, `<p><b>Lịch sử kết nối từng lần — %s (tổng %d lần cắm)</b></p>`,
                        r.VendorModel, r.PlugCount)
                max := len(r.Sessions)
                if max > 20 {
                        max = 20
                }
                for k := 0; k < max; k++ {
                        ...
                        fmt.Fprintf(&sb, `<p style="margin-left:24pt;margin-top:2pt;margin-bottom:2pt;">Lần %d: cắm %s — rút %s (thời lượng: %s)</p>`,
                                k+1, s.Arrival, rem, orDash(s.Duration))
                }
                if len(r.Sessions) > 20 {
                        fmt.Fprintf(&sb, `<p style="margin-left:24pt;">... và %d lần nữa.</p>`, len(r.Sessions)-20)
                }
        }
        sb.WriteString(`<h3>Lệnh xử lý</h3>`)
