...
func listSuspiciousDirs() []string {
        ...
        dirs := []string{
                // Downloads của user (nơi hay giấu crack nhất)
                ...
                // Temp của user (keylogger hay nằm đây)
                filepath.Join(userProfile, "AppData", "Local", "Temp"),
                // Roaming (nơi keylogger hay cài)
                filepath.Join(userProfile, "AppData", "Roaming"),
                // Local AppData (một số crack cài vào đây)
                filepath.Join(userProfile, "AppData", "Local"),
                // ProgramData (chứa folder của KMSpico, KMSAuto...)
                `C:\ProgramData`,
                // ⚠️ BỎ C:\Program Files và C:\Program Files (x86)
                // vì có quá nhiều file (50,000+), làm chậm quét 30-60s
        }
